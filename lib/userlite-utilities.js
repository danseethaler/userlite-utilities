    ...
    activate: function(state) {
        ...
        atom.commands.add('atom-workspace', {
            ...
            'userlite-utilities:GetS3Path': function() {
                ...
                rootLength = 'master'.length;

                if (currentPath.substr(0, rootLength) === 'master') {

                    currentPath = currentPath.substr(rootLength);
                ...
                rootLength = 'projects'.length;

                if (currentPath.substr(0, rootLength) === 'projects') {
                ...
                if (currentPath.substr(0, rootLength) === 'root') {

                    currentPath = currentPath.substr(rootLength);
                    currentPath = currentPath.substr(0, currentPath.lastIndexOf('/') + 1);

                    var s3Path = "https://console.aws.amazon.com/s3/home?region=us-east-1#&bucket=codeversion&prefix=dev" + currentPath;
                ...
                rootLength = 'root'.length;
                'establish_balance/eb_home/frontend/mods/app/nav.php';
                'projects/training/v999/uMod/triks/processExpirations.php';

                if (currentPath.indexOf('frontend') !== -1) {
                    currentPathArray = currentPath.split('/');

                    if (currentPathArray.length > 4) {
                        var builtPath = currentPathArray[1].replace('_', '') + '/v999/uMod/lib';

                        for (var i = 3; i < currentPathArray.length - 1; i++) {
                            builtPath = builtPath + '/' + currentPathArray[i];
                        }

                        var s3Path = "https://console.aws.amazon.com/s3/home?region=us-east-1#&bucket=codeversion&prefix=dev/projects/apps/userlitestoreapps/" + builtPath;
                ...
                if (currentPath.indexOf('backend') !== -1) {
                    ...
                    if (currentPathArray.length > 4) {
                        ...
                        var app = currentPathArray[1].replace('_', '');
                        var appUpper = app.charAt(0).toUpperCase() + app.slice(1);

                        var builtPath = app + '/v999/uLib/' + appUpper;

                        for (var i = 3; i < currentPathArray.length - 1; i++) {
                            builtPath = builtPath + '/' + currentPathArray[i];
                        }

                        var s3Path = "https://console.aws.amazon.com/s3/home?region=us-east-1#&bucket=codeversion&prefix=dev/master/apps/userlite/storeapps/" + builtPath;
            ...
            'userlite-utilities:Cleanup': function() {
                ...
                // This next line breaks the editor for some reason
                // editor.buffer.replace(/((\ +|\t)+\n)+/g, '\n');
